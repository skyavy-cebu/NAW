...
class AdminUserProfileForm extends BaseProfileForm{
  ...
  public function validateForm($validator, $values){
    ...
    if($values['email1'] != $values['email2']){
      $error = new sfValidatorError($validator, 'Email didn\'t match');
      $error_data['email1'] = $error;
    }
    
    if($values['pass1'] != $values['pass2']){
      $error = new sfValidatorError($validator, 'Password didn\'t match');
      $error_data['pass1'] = $error;
    }
